...
- `Change FPS target` - Shows up only in docked mode. Shows up a table with different FPS values, from 15 to 60 by default, with the possibility of expanding to 120 FPS.
- `Disable custom FPS target` - Removes FPS Target. Since we cannot predict what interval mode is expected at this point, it is in the user's discretion to manipulate FPS to bring back the correct interval before disabling the FPS target.
- `Advanced settings` - Submenu which consists of:
  - If the game is using NVN
    - `Window Sync Wait` - This is a dangerous setting that disabled can crash the game, but in some cases can bring the benefit of disabling double buffer vsync at the cost of small graphical glitches (check the list of games compatible with this solution at the bottom of README). Use it with caution. It won't show if the game is not using a double buffer. 
    - `Set Buffering` - If the game is using any buffering other than double, this option will show that will allow you to force the game to run at any other buffering that is not higher than the original one (so, e.g., you cannot change double buffer to triple buffer). Lowering the buffer is recommended only for games that have near-perfect performance at 30 or 60 FPS, but suffer from bad framepacing or big input lag. If you force a double buffer in games with uneven performance, FPS drops will be very severe. It can be applied only at the boot of the game, so after changing buffering, you must save settings in FPSLocker and restart the game. <br> Explanation of `Set/Active/Available Buffers`: 
      - Set - How many buffers were set by using `nvnWindowSetNumActiveTextures`. If the game is not using it, it will be 0. It can be used by games to set a lower buffer value than the reserved space allows. If this is detected to be used and lower than Available Buffers, you can use the "(force)" variant next to the default option. Without `(force)`, it will reset to default settings.
      - Active - How many buffers are used by the game. 
      - Available - How many buffers are provided to NVN. We can use this information to force games to utilize all buffers when they are not doing it.
  - If the game is using Vulkan
    - `Set Buffering` - Switch between double buffer and triple buffer. Lowering the buffer amount is recommended only for games that have near-perfect performance at 30 or 60 FPS, but suffer from bad framepacing or big input lag. If you force a double buffer in games with uneven performance, FPS drops will be very severe. It can be applied only at the boot of the game, so after changing buffering, you must save settings in FPSLocker and restart the game.
  - `Convert config to patch file` - If a proper config file exists for this game and version, you will get an option to convert it to a patch file that will be loaded when you run this game next time. Patch is saved to `SaltySD/plugins/FPSLocker/patches/*titleid_uppercase*/*buildid_uppercase*.bin`
  - `Delete patch file` - If a proper config file exists for this game and version, you will get an option to delete the patch file so it won't be loaded when you run this game next time.
  - `Check/download config file` - Checks in Warehouse repository if config for this game and version exists. If it exists, it is downloaded and also checked if it's the same as the one on the SD card. If it's not, the overlay will remove the existing patch and config file, and the user must manually convert the new config to a patch file. 0x312 error means we got an unexpected file from GitHub. Any other error code means that something is happening with your connection or the GitHub server.
  - `Halt unfocused game` - Some games are not suspended when your Switch is in the home menu. Enabling this option moves into the infinite loop game's thread responsible for pushing frames to display when in the home menu. Enabling this option can cause various issues depending on the game, so be sure not to use it frivolously.
- `Display settings` - Submenu related to display refresh rate. Consists of:
  ...
  - `Decrease refresh rate` - Shows up only in handheld mode. Change the display refresh rate down to 40 Hz. In OLED units it's blocked.
  - `Change refresh rate` - Shows up only in docked mode. Choose the display refresh rate from the list.
  - `Handheld Display Sync`/`Docked Display Sync` - When turned on, all three options above are not available, display refresh rate is changed only when the game is running, and matches the refresh rate with FPS Target.
  - `Retro Remake Mode` - This option shows only for people that use Lite with screen `InnoLux 2J055IA-27A (Rev B1)` or `Retro Remake SUPER5` (first revision only). That is because Retro Remake displays require a special approach to change refresh rate, and the first version of SUPER5 is spoofing the ID of an already existing display, which makes it impossible to detect which one is in use, so the user must manually enable it if they are using a SUPER5 display. All other Retro Remake displays are detected automatically.
  - `Docked Settings` - Submenu related to the display refresh rate of external displays. Not accessible for Lite units. Consists of:
      - `myDP link rate` - It will report `HBR` or `HBR2` mode. HBR mode doesn't allow going higher than 75 Hz at 1080p. More at the bottom of the readme.
      - `Config ID` - What is the name of the config file used to store settings for your currently connected display. You can find the file in the `SaltySD/plugins/FPSLocker/ExtDisplays` folder.
      - `Allowed refresh rates` - You can check and edit manually which refresh rates are enabled for the currently connected external display. It consists of 40, 45, 50 and 55 Hz. By default, 50 is turned on; everything else is turned off.
      - `Display underclock wizard` - It goes automatically through refresh rates from 40 to 55, the user is asked to press the required button to confirm it's working, if not pressed for 15 seconds, it goes to the next refresh rate. After checking all refresh rates, you are moved to the `Allowed refresh rates` menu to check results.
      - `Display overclock wizard` - It shows only if the external display reported max refresh rate is equal to or above 70 Hz. Goes automatically through refresh rates from 70 to max your display supports, with a cap being 120 Hz, the user is asked to press the required button to confirm it's working, if not pressed for 10 seconds, it goes to the next refresh rate. After checking all refresh rates, you are moved to the `Allowed refresh rates` menu to check results.
      - `Frameskip tester` - It allows you to check if your display is showing the currently used signal at native refresh rates. Many displays may support, for example 50 Hz, but they are still displaying stuff at 60 Hz. Instructions on how to use it are provided when this menu is selected. This menu is also available in handheld mode.
      - `Additional settings` - Submenu with options related to how FPSLocker/FPSLocker patches are working in docked mode. Currently, you can choose from:
          - `Allow patches to force 60 Hz` - Some FPSLocker patches are forcing 60 Hz to fix framepacing issues with 30 FPS cutscenes. When such a change happens, the game is paused for 4 seconds before continuing. By default, it is turned on. Turning it off will apply only FPS lock without changing the refresh rate and without delay.
          - `Use lowest refresh rate for unmatched FPS targets` - For example, for a 60 Hz display, a 35 FPS target may not have an available refresh rate matching it. By enabling this option, you will get the lowest enabled refresh rate in the `Allowed refresh rates` menu. This option is disabled by default, which will result in setting 60 Hz in that case.
...
- `Force English language` - If you prefer using English, this option will force the overlay to use it. It is achieved by a self-modifying executable, so after updating the overlay to a newer release, it will be turned off.
