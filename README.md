...
- Advanced settings - submenu which consists of:
  ...
  - `Check/download config file` - Checks in Warehouse repository if config for this game and version exists. If exists, it also checks if it's the same as the one on sdcard. If it is, it's removing existing patch file and user must manually convert new config to patch file. If patch doesn't exist in repository for this game and version, you should get `Err 404`. 0x312 error means we got unexpected file from github. Any other error means that something is happening with your connection or github server.
