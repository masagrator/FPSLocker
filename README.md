...
- `Display settings` - submenu related to display refresh rate. Consists of:
  ...
  - `OLED 60hz stock colors when possible` - this option shows only for Switch OLED users. SaltyNX by default avoids overriding panel colors at 60 Hz unless user changes refresh rate, then it's not possible to restore original colors until Switch goes to sleep mode. Disabling this option always overrides colors to the one stored by SaltyNX. Some people may prefer how colors are adjusted here which prefers more red tint over green tint at lower half of brightness. After changing setting, while panel is set to 60 Hz refresh rate, you won't see the difference until you go to sleep mode.
